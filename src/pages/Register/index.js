...
import { registerItems } from '../../utils/constants'
...
const SignupSchema = Yup.object().shape({
    ...
    matKhau: Yup.string()
        ...
        .required('Không bỏ trống trường này'),
    email: Yup.string().email('Email không hợp lệ').required('Không bỏ trống trường này'),
    hoTen: Yup.string()
        .max(50, 'Họ tên vượt quá ký tự cho phép')
        .required('Không bỏ trống trường này'),
    soDt: Yup.string()
        ...
        <section className='flex justify-center pb-20'>
            <div className="w-4/5 flex flex-col items-center">
                ...
                <div className="w-full max-w-md p-8 space-y-3 rounded-xl dark:bg-gray-900 dark:text-gray-100 border-2 border-pink-600">
                    <h1 className="text-2xl font-bold text-center text-pink-600">Đăng ký</h1>
                    <form onSubmit={formik.handleSubmit} className="space-y-6">
                        ...
                        <button type='submit' className="block w-full p-3 text-center rounded-lg text-white font-bold dark:text-gray-900 dark:bg-red-400 bg-pink-600 hover:bg-pink-700 duration-300">Đăng ký</button>
                    </form>
                    <p className="text-xs text-center sm:px-6 dark:text-gray-400">Đã có tài khoản?
                        <a rel="noopener noreferrer" href="#" className="underline dark:text-gray-100 text-pink-600 font-semibold">Đăng nhập ngay</a>
                    </p>
                </div>
            </div>
        </section>
